...
//    ,'<li title="更多" layim-event="tab" lay-type="more"><i class="layui-icon">&#xe670;</i><span>更多</span><i class="layim-new" id="LAY_layimNewMore"></i></li>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ...
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
		,'<span class="layui-icon layim-tool-audio" title="语音通话" layim-event="startaudio">&#xe6fc;</span>'
		,'<span class="layui-icon layim-tool-video" title="视频通话" layim-event="startvideo">&#xe6ed;</span>'
        ,'<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face">&#xe60c;</span>'
        ,'{{# if(d.base && d.base.uploadImage){ }}'
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" multiple accept="image/*"></span><input type="button" id="btnHide" class="none">'
        ,'{{# }; }}'
        ...
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'
		 ,'<span class="layui-icon layim-tool-location" style="display: none;" title="位置" layim-event="location">&#xe715;</span>'
		 ,'<span class="layui-icon layim-tool-face" title="评价" layim-event="rate">&#xe67b;</span>'
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    ,obj = {
      ...
      cache = $.extend(cache, obj);
      //popim(laytpl(comTpl(elemTpl)).render(obj));
	  var popchat_id=chater.loginname;
...
//			 }, 500);
  ...
  var pushChatlog = function(message){
    ...
    if(chatlog[message.type + cache.mine.id]){
      chatlog[message.type + cache.mine.id].push(message);
      if(chatlog[message.type + cache.mine.id].length > MAX_ITEM){
        chatlog[message.type + cache.mine.id].shift();
      }
    } else {
      chatlog[message.type + cache.mine.id] = [message];
    }
	//alert(cache.mine.id);
  ...
  var viewChatlog = function(){
    ...
    layui.each(chatlog[thatChat.data.type + cache.mine.id], function(index, item){
      ...
      layui.upload.render({
		...
		,choose: function(obj) {
			...
			for(var i = 0; i < indexArr.length; i++) {
				if(files[indexArr[i]].size > 200 * 1024) {
					if (!notSupport) {
					  (function(i) {
						new html5ImgCompress(files[indexArr[i]], {
						  before: function(file) {
							  ...
							  if(k==indexArr.length) $("#btnHide").trigger("click");
						  },
						  fail: function(file) {
						  },
