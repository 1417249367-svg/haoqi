	...
	var handlers = {
		...
		onSelectError : function(file, errorCode, errorMsg) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					this.queueData.errorMsg += '\n选定的文件 "' + file.name
							+ '" 是不允许的文件类型 (' + settings.fileTypeDesc + ').';
					break;
				}
			}
...
//			this.removePostParam("existname");
//			if(this.queueData.filesSelected==1) settings.checkExisting='/public/cloud.php?op=doc_checkExisting';
//			else settings.checkExisting=false;
			if(settings.checkExisting) {
				var data = {'loginname':curr_loginname,'password':curr_password,'parent_type':parent_type,'parent_id':parent_id,'root_type':root_type,'root_id':root_id,'filename':file.name,'flag':'swf'} ;
				$.ajax({
					type : 'POST',
					dataType:"json",
					async : false,
...
//								break;
							case 2:
								if(result.msg){
									...
									if (overwrite) {
										_this.addPostParam("existname",result.msg);
									}
								}
								break;
						}
...
//console.log('wu:'+JSON.stringify(_this.settings.formData));
			// Call the user-defined event handler
