...
{
	var param = {point:0,label:"messengkefuclot",myid:my.userid,chater:"","Msg_IDs4":ArrMsg[10],ispage:0};
    var url = getAjaxUrl("msg","list") ; 
    $.getJSON(url,param , function(result){
		if (result.status == undefined)
		{
...
function getLeaveClotMesseng()
{
	...
	var param = {point:0,label:"messengkefuclot",myid:my.userid,chater:"",ispush:ispush,ispage:0};
    var url = getAjaxUrl("msg","list") ; 
...
//function getChaters()
//{
//	var param = {point:0,label:"messengkefu_getchaters",myid:my.userid,ispage:1};
//    var url = getAjaxUrl("msg","list") ; 
//	//document.write(url+JSON.stringify(param));
//    $.getJSON(url,param , function(result){
//		if (result.status == undefined)
...
//
//
//function getClotChaters()
//{
//	var param = {point:0,label:"messengkefuclot_getchaters",myid:my.userid,ispage:1};
//    var url = getAjaxUrl("msg","list") ; 
//	//document.write(url+JSON.stringify(param));
//    $.getJSON(url,param , function(result){
//		if (result.status == undefined)
		...
		switch (parseInt(result.error))
		{
			case 0:
			   ...
			   //$(currWin).attr("loginname",chater.loginname);
				...
				//console.log('connectType:'+chater.connectType);
...
//					}
			...
			case 2:
				chater.issilence = 1;
				//location.href=location.href.replace("index.html","message.html")+"&userid="+my.userid+"&chater="+chater.loginname;
				break; 
			 default:
				myAlert(result.msg) ;
				break ;
    ...
    $.getJSON(url,param , function(result){
		...
		switch (parseInt(result.error))
		{
			...
			case 2:
			    chater.lv_chater_ro_to_type = 0;
				chater.issilence = 1;
				//location.href=location.href.replace("index.html","message.html")+"&userid="+my.userid+"&groupid="+typeid;
				break; 
			case 3:
...
//
//    if(chater.lv_chater_ro_to_type) asideUrl = "aside_vister1.html" ;
...
function closeChat(closeRole)
{
    ...
    if (closeRole == 2)
    {
	   var obj = {msg_id:0,fcname:langs.lv_chat_system,to_type:3,usertext:langs.lv_chat_close1,isreceipt:0,isAppend:1,myid:msg.from_uid,picture:"",time:"",objtype:0,lv_chater_ro_to_type:0,pid:0,typename:""};
	   printMsg(obj);
    }
...
function recvMsg(msg)
{
    ...
    $.getJSON(url,param , function(result){
		if (result.status == undefined)
		{
			...
			//console.log(JSON.stringify(msg)+'|'+JSON.stringify(result));
			if(result.myid==my.userid){
				 var To_Type = parseInt(result.to_type);
				 if(result.youid!=chater.loginname) return ;
			}else{
...
{
	var param = {point:0,msg_id:msg.content,myid:my.userid,clotid:""};
