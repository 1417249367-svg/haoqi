...
var UDCAPTURE_SETUP = "screenhot/UdCapture.msi"; //安装文件
...
if (supportActiveX && (window.navigator.platform == "Win64" || window.navigator.cpuClass == "x64"))//64位浏览器安装文件
    UDCAPTURE_SETUP = "screenhot/UdCapture64.msi";
    ...
    if (supportActiveX) {//if IE               
        ...
        if (axObj.PostUrl != undefined) {
            if (f_hasNewVer(axObj.GetVersion())) {
                if (confirm("在线屏幕截图控件有新版本，升级后才能使用！\r\n点确定进行升级，升级时需关闭浏览器窗口...\r\n如果您已经升级安装,请关闭后重新打开浏览器...")) {
                    f_startSetup();
                }
                return false;
            }
    ...
    else if (navigator.plugins)//NP
    {
        ...
        if (plugin) {
            ...
            if (f_hasNewVer(pluginVersion)) {
                if (confirm("在线屏幕截图插件有新版本，升级后才能使用！\r\n点确定进行升级...")) {
                    f_startSetup();
                }
                return false;
            }
...
//开始屏幕截图
function f_capture(autoMin,captureScreen) {
    if (autoMin == undefined)
        autoMin = false ;
    if (captureScreen == undefined)
        captureScreen = false ;
        
    if (f_installCheck()) {
        udCapCtl.AutoMinimize = autoMin;
