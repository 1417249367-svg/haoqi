<?php
	...
	if (count($data)) {
		...
		if((int)$data["formfiletype"]==7&&strrchr($filename, '.')==".pdf"){
			$url = phpescape(phpescape(getRootPath()."/public/cloud.html?op=getpdffile&name=".phpescape($filpath)."&loginname=".phpescape(CurUser::getLoginName())."&password=".CurUser::getPassword()));
//			$url = getRootPath()."/public/cloud.html?op=getpdffile&name=".phpescape($filpath)."&loginname=".phpescape(CurUser::getLoginName())."&password=".CurUser::getPassword();
//			echo $url;
//			exit();
//			header("Location:".getRootPath()."/public/cloud.html?op=getpdffile&name=".$filpath."&loginname=".phpescape(CurUser::getLoginName())."&password=".CurUser::getPassword());
			header("Location:/vendor/pdfjs/web/viewer.html?formfiletype=2&col_id=0&filename=".$filename."&file=".$url."&loginname=".phpescape(CurUser::getLoginName())."&password=".CurUser::getPassword());
			die();
		}
		...
		$doc = new Doc();
		switch ((int)$data["formfiletype"]) {
			case 1 :
				$sql = " select * from PtpFile where OnlineID=".$onlineid;
				$row = $db -> executeDataRow($sql);

				$path_text = "" ;
				$path_data = $doc -> get_path_data(DOC_VFOLDER,$row["ptpfolderid"]);
				foreach($path_data as $node)
				{
					$name = base64_encode(mb_convert_encoding($node["doc_name"],'GBK','UTF-8')) ;
					$path_text = $name."_".$node["doc_type"]."_".$node["col_id"]."_0" . ($path_text?"/":"") . $path_text ;
				}

				if ((int)$row["to_type"] == 1){
					$path_text="#public/".$path_text;
					if (DocAce::can1(DOCACE_UPDATE,DOC_FOLDER,$row["ptpfolderid"],$root_id,1)){
						 $ac = "view";
						 if (DocAce::can1(DOCACE_EDIT,DOC_FOLDER,$row["ptpfolderid"],$root_id,1)) $ac = "edit";
					}else $ac = "no";
				}else $path_text="#person/".$path_text;

				$path_text = getRootPath().'/cloud/include/yunpan.html'.$path_text;
//				echo $path_text;
//				exit();
				break;
			default:
				$path_text = "http://www.haoqiniao.cn" ;
				break ;
		}
		...
		if($ac=="edit"){
			switch ((int)$data["formfiletype"]) {
				case 1 :
					...
					$row = $db -> executeDataRow($sql);
					if ((int)$row["to_type"] == 1){
						if (! DocAce::can1(DOCACE_DOWNLOAD,DOC_FOLDER,$row["ptpfolderid"],$root_id,1)) $download = "false";
					}else{
						if (! DocAce::can3(DOCACE_DOWNLOAD,$parent_type,$parent_id,$root_id)) $download = "false";
					}
					break;
				case 2 :
					if (! DocAce::can3(DOCACE_DOWNLOAD_LEAVEFILE,$parent_type,$parent_id,$root_id)) $download = "false";
					break;
				case 4 :
					if (! DocAce::can3(DOCACE_DOWNLOAD_CLOTFILE,$parent_type,$parent_id,$root_id)) $download = "false";
					break;	
			}
		}
		...
		docEditor = new DocsAPI.DocEditor("iframeEditor",
			{
				...
				document: {
					title: fileName,
					url: "<?php echo serverPath(TRUE)."/public/cloud.html?op=getfile&label=onlinefile&myid=1&id=".$onlineid ?>",
					fileType: fileType,
					...
					permissions: {
						comment: false,
						copy: <?php echo $download?>,
						download: <?php echo $download?>,
						print: <?php echo $download?>,
						edit: true,
						review: true
					}
				},
