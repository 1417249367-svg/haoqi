<?php
	...
	if (isset($_REQUEST['filpath'])){
		...
		$row = $db -> executeDataRow("select top 1 * from OnlineFile where MyID='".$userId."' and FilPath='".$filpath."' order by ID desc");
...
echo '<span class="title item">'.$filename.(($ac=="view") ? '(只读)' : '').'</span>';
echo '<span class="time item">'.$todate.'</span>';
echo '</div>';
echo '<div class="right">';
echo '<ul class="nav navbar-nav pull-right" id="nav_account">';
if ($userId == 0){
echo '<li><a>当前仅能查看,登录后可获得更多操作权限</a></li>';
echo '<li class="dropdown" id="login_btn">';
echo '<a href="javascript:void(0);" class="dropdown-toggle"><img src="../assets/img/avatar.png" class="photo"> 登录</a>';
echo '</li>';
}else{
if($myid==$userId) echo '<li><a href="javascript:void(0);" onclick="doc_set(\''.DOC_FILE.'_'.$onlineid.'_0\',this);">权限</a></li>';
echo '<li><a href="javascript:void(0);">收藏</a></li>';
...
echo '<li id="logout_btn"><a href="javascript:void(0);">退出</a></li>';
echo '</ul>';
echo '</li>';
}
echo '</ul>';
...
$sql = "Select * from OnlineRevisedFile where OnlineID=" . $onlineid . " order by ID desc";
...
//	$default_dir = str_replace("/","\\",$default_dir) ;	
	...
	$history .= '{"changes":'.json_encode($callback["history"]["changes"]).',"created":"'.$callback["lastsave"].'","key":"'.$callback["key"].'","serverVersion":"'.$callback["history"]["serverVersion"].'","user":{"id":"2","name":"杨汉周"},"version":"'.($i-1).'"},';
	$i -= 1;
}
...
//		    });
	...
	var сonnectEditor = function () {
		...
		//echo getStoragePath($filpath);
			if ((!file_exists(getStoragePath($filpath)))||$ac == "not") {
				...
				echo '$("#file_empty .title").html("文件不存在");';
				echo 'return;';
			}
			if ($ac == "no") {
				echo '$("#iframeEditor").hide();';
				echo '$("#file_empty").show();';
				echo '$("#file_empty .title").html("您没有权限查看");';
				echo 'return;';
			}
		?>
		var type = new RegExp("<?php echo $GLOBALS['MOBILE_REGEX'] ?>", "i").test(window.navigator.userAgent) ? "mobile" : "desktop";
	...
	var сonnectEditor1 = function (num) {

		<?php
		//echo getStoragePath($filpath);
			if ((!file_exists(getStoragePath($filpath)))||$ac == "not") {
				echo '$("#iframeEditor1").hide();';
				echo '$("#file_empty1").show();';
				echo '$("#file_empty1 .title").html("文件不存在");';
				echo 'return;';
			}
			if ($ac == "no") {
				echo '$("#iframeEditor1").hide();';
				echo '$("#file_empty1").show();';
				echo '$("#file_empty1 .title").html("您没有权限查看");';
				echo 'return;';
			}
		?>
		if(num==pageindex) return;
		pageindex = num;
		$("#sidebar li").removeClass("active");
		$("#100_"+pageindex+"_0").addClass("active");
		$("#toolbartitle").html($("#100_"+pageindex+"_0").attr("data-name"));
		if(pageindex==$("#sidebar .submenu li").length){
			 $(".btn-primary").hide();
			  revisedFile = new DocsAPI.DocEditor("iframeEditor1",
				  {
					  width: "100%",
					  height: "100%",
					  type: "embedded",
					  ...
					  document: {
						  title: fileName,
						  url: "<?php echo $GLOBALS['EXAMPLE_URL']."/Data/".str_replace("\\","/",$filpath) ?>",
						  fileType: fileType,
						  key: "<?php echo getDocEditorKey($myid,$filename,$filpath) ?>",
						  info: {
							  author: "<?php echo $usname ?>",
			 ...
			 $(".btn-primary").show();
			 alert($("#100_"+pageindex+"_0").attr("data-changesUrl")+"|"+$("#100_"+pageindex+"_0").attr("data-key")+"|"+$("#100_"+pageindex+"_0").attr("data-url")+"|"+$("#100_"+pageindex+"_0").attr("data-sid"));
			  revisedFile.setHistoryData({
//				  "changesUrl": $("#100_"+pageindex+"_0").attr("data-changesUrl"), //the changesUrl from the JSON object returned after saving the document
				  "key": $("#100_"+pageindex+"_0").attr("data-key"),
//				  "previous": {
//					  "key": $("#100_"+pageindex+"_0").attr("data-key"),
//					  "url": $("#100_"+pageindex+"_0").attr("data-url")
//				  },
				  "url": $("#100_"+pageindex+"_0").attr("data-url"),
				  "version": $("#100_"+pageindex+"_0").attr("data-sid")
			  })
		}
