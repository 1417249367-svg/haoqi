<?php  require_once("../include/fun.php");?>
           <form id="form-group" method="post"  class="form-horizontal"   data-obj="cloud"  data-table="OnlineFile" data-fldid="OnlineID" data-fldname="TypePath" data-label="onlinefile" enctype="multipart/form-data">
...
<!--                    <ul id="tabs" class="nav nav-tabs">
                    ...
                    <div class="tab-content" style="display:block;" id="base">
                        <div id="member1" class="form-group clearfix">
						    <div class="bor" style="overflow:scroll;margin:0 50px 0 50px;height:100px;">
							    <table id="tbl_member1" class="table" style="margin:0px;padding:0px;">
								    <thead>
									    <!--<tr><td style="width:250px;"></td><td style="width:100px;"></td><td></td></tr>-->
								    </thead>
								    <tbody>
<!--                                    <tr id="user_4" data="4"><td class="col_name">FBA16ZRCS1PS<input type="hidden" name="col_filelist" /></td><td><span style="cursor:pointer;margin-left:10px;" onclick="deleteMember(4)"><img src="/static/img/delete_6.gif" width="16" height="16" /></span></td></tr>-->
								    </tbody>
							    </table>
						    </div>
...
<!--                <input type="hidden" id="tagIds" name="tagIds" />-->
                ...
                <input type="hidden" id="memberIds1" name="memberIds1" />
...
var curr_userid = "<?=CurUser::getUserId()?>" ;
...
function doc_detail_init()
{
	...
	group_getCallBack();
	dataForm = $("#form-group").attr("data-id",doc_id).dataForm({savecallback:group_saveCallBack});
...
function group_getCallBack()
{
    $("input[name='authority1'][value='0']").attr("checked",true);
    $("input[name='authority2'][value='0']").attr("checked",true);
	$("input[name='authority3'][value='1']").attr("checked",true);
    doc_html_init("authority2",0);
    doc_html_init("authority1",0);
...
//		addMember(data.members[i].col_id,data.members[i].col_name,data.members[i].col_loginname,data.members[i].authority);
	...
	for(var i=0;i<doc_data.length;i++)
		addMember1(doc_data[i].col_id,doc_data[i].col_name,doc_data[i].usname);
...
function saveForm()
{
	...
	$("#memberIds1").val(getMemberIds1());
	if (($("#memberIds1").val() == "") || ($("#memberIds1").val() == undefined))
	{
		myAlert(langs.onlinefile_text);
		return ;
	}
    dataForm.save();
}
...
function addMember1(col_id,col_name,usname)
{
	var row = $("<tr id='doc_" + col_id + "' data='" + col_id + "'><td class='col_name'>" + col_name + "</td><td class='col_loginname'>" + usname + "</td><td><a href='#' onclick=\"deleteMember1(" + col_id + ")\">"+langs.btn_delete+"</a></td></tr>") ;
	$("#tbl_member1 tbody").append(row);
}

function deleteMember1(userId)
{
	$("#doc_" + userId).remove();
}


function getMemberIds1()
{
	var memberIds = "" ;
	$("#tbl_member1 tbody tr").each(function(){
		if (memberIds != "")
			memberIds += "," ;
