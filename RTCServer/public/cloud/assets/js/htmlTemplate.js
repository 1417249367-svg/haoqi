...
function fileConstruct(fileData) {
    var str = '<div class="item" data-file-id="${fileData.id}">'
                    ...
                    +'<div class="file-img">'
                        +'<img src="/cloud/assets/img/ico/folder.png"/>'
                    +'</div>'
...
function fileConstruct1(fileData) {
    var str = '<div class="item" data-file-id="${fileData.id}">'
                    +'<lable class="checkbox checkbox1"></lable>'
                    +'<div class="file-img1">'
                        +'<img src="/cloud/assets/img/ico/folder-32.png"/>'
                    +'</div>'
                    +'<div class="file-title-box1">'
                        +'<span class="file-tilte">${fileData.title}</span>'
                        +'<span class="file-edtor">'
                            +'<input type="text" value="${fileData.title}" class="edtor">'
                        +'</span>'
                    +'</div>'
                    +'<div class="dtime">'
                        +'<span class="file-dtime"><span>2020-9-1 12:00:00</span><span>152KB</span></span>'
                        +'<span class="file-edtors"><i class="ico ico-edit" title="编辑"></i><i class="ico ico-down" title="下载"></i><i class="ico ico-share" title="分享"></i><i class="ico ico-move" title="移动"></i><i class="ico ico-rename" title="命名"></i><i class="ico ico-del" title="删除"></i><i class="ico ico-qrcode"></i></span>'
                    +'</div>'
               +'</div>';
    return str;
}
...
function getFileHtml(fileData) {
    var fileHtml = '<div class="file-item">${fileConstruct(fileData)}</div>';
    return fileHtml;
}

function getFileHtml1(fileData) {
    var fileHtml = '<div class="file-item file-item1">${fileConstruct1(fileData)}</div>';
    return fileHtml;
}
...
function createFilesHtml(fileData, renderId) {
    ...
    childData.forEach(function (item) {
        if(arrayBtn.onOff) fileHtml += getFileHtml(item);
		else fileHtml += getFileHtml1(item);
    });
    return fileHtml;
}
...
function createPathNavHtml(fileData, fileId) {
    ...
    parents.forEach(function (item, index) {
        ...
        pathNavHtml += '<a href="javascript:void(0)" style="z-index: ${lv--};" data-file-id="${item.id}">${item.title}</a>';
    });
    // 渲染当前文件路径的最后一个
    pathNavHtml += '<span class="current-path" style="z-index: ${lv--};" data-file-id="${parents[parents.length-1].id}">${parents[parents.length-1].title}</span>';
    return pathNavHtml;
}
