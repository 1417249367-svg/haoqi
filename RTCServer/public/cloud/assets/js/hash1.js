/**
 * 锚点动作的URL定位
 * 1)生成当前目录信息 root_type,root_id,parent_type,parent_id
 * 2) 生成导航地址 path
 
 * @date    20141205
 * 
 */
var DOC_FOLDER 		= 105 ;
var hash = {"url":"","param":""};
var ispath = 0 ;
var path  = "" ;
var query = "" ;
var path_data = [] ;  //来用权限判断 [{"doc_type":105,"doc_id":1,"root_id":1},{"doc_type":102,"doc_id":1,"root_id":1}]
var power = -1 ; //当前目录的权限

function hash_init()
{
	$(window).bind("hashchange",function(){
		load_data();
	});
	
	load_data();
}

function hash_to(name,param)
{
	if (param == undefined)
		param = "" ;
		
	hash.url = hash.url + "/" + name ;
	hash.param = param ;
	var url = hash.url;
	if (hash.param != "")
		url += "?" + hash.param ;
	location.href = url ;
}

// all/test?id=1_1
function hash_get(url)
{
	var arr = url.split("?");
	hash.url = arr[0] ;
	if (arr.length>1)
		hash.param = arr[1] ;
	return hash ;
}

//加载数据
function load_data(url)
{
	if (url == undefined)
		url = location.hash ; 
	if(url == "#")
		return false;
	if (url == "")
		url = "#public" ;
//	if (url == "#person")
//		url = "#个人文档_102_" + my_root_id + "_" + my_root_id;

	//得到HASHURL
	hash = hash_get(url) ;
	//console.log(JSON.stringify(hash));
	var arr_url = hash.url.replace("#","").split("/") ;
	
	//创建地址，并生成相关数据parent_type,root_id...
	nav_getname(hash.url.replace("#",""));
	path_create(arr_url);
	
	//不能放到 doc_list,因为delete也会 doc_list
	if (show_type == "thumb")
		show_thumb();
	else
		show_list();
		
	doc_list_init();
	doc_list();
	doc_dropzone();
}



//创建导航路径
function path_create(arr_url)
{
	power = -1 ;
	path_data = [] ;
	root_type = 0 ;
	root_id = 0 ;
	parent_type = DOC_FOLDER ;
	parent_id ="0" ;
	file_type = 0 ;
	key = "" ;
	
	var path_html = "";
	label = arr_url[0] ;
	ispath = path_valid(arr_url);
	path = "" ;
	var index=arr_url.length;
	
	for(var i=0;i<arr_url.length;i++)
	{
		var arr_item = arr_url[i].split("_");
		
		//首项 取root_type
		if (i == 0)
		{
			if (arr_item[0] == "public")
				root_type = 1 ;
			if (arr_item[0] == "person")
				root_type = 3 ;
		}
		
		
		
		//最后一项 取parent_type,parent_id
		if (i == arr_url.length-1)
		{
			//按路径查询  name_doctype_docid_rootid
			if (arr_item.length >= 4) 
			{
				parent_type = arr_item[1] ;
				parent_id = arr_item[2] ;
				root_id = arr_item[3] ;

			}
			

		}
		
		//添加到路径
		if (arr_item.length >=4)
			path_data.push({"doc_type":arr_item[1],"doc_id":arr_item[2],"root_id":arr_item[3]});  

		//得到地址HTML 累加
		path += (path == ""?"":"/") + arr_url[i] ;
		
		var name = "" ;
		if (path_valid(arr_url,i))
			name = decodeURI(decode64(arr_item[0])) ;
		else
			name = path_getname(arr_item[0].split("-")[0]) ;
			//console.log(decode64(arr_item[0]));
		if (i < arr_url.length-1)
		    path_html += '<a href="#' + path + '?' + hash.param + '" style="z-index: ' + index + ';">' +  name + '</a>' ;
		else
		    path_html += '<span class="current-path" style="z-index: ' + index + ';">'+name+'</span>' ;
		index--;

	}
	//document.write(JSON.stringify(path_data));
	//按类型查询
	if (! ispath) 
	{
		if (label == "search"){
			var arr_item = arr_url[1].split("-");
			key = decode64(arr_item[0]) ;
			root_type = arr_item[1] ;
			parent_id = arr_item[2] ;
		}else{
			if (parseInt(arr_url[1]))
				file_type = arr_url[1] ;
		}
	}

	$(".path-nav").html(path_html);

//	var lis= $("#path ul li");
//	$("#path ul li").eq(lis.length-1).addClass("active");  //最后一项为选中
//	
//	if (lis.length>1)
//		$("#path ul li").eq(lis.length-2).addClass("prev");  //前一项设置特殊的样式
	
	if ($("#search_key").val() != key)
		$("#search_key").val(key);
		
	//加入路径，服务端不用再取路径，提高效率
	var curr_path = "";

	for(var i=path_data.length-1;i>=0;i--)
		curr_path += (curr_path == ""?"":",") + path_data[i].doc_type + "_" + path_data[i].doc_id + "_" + path_data[i].root_id ;

	//每个ajax都带个这个，以便权限判断
	query = "parent_type=" + parent_type + "&parent_id=" + parent_id + "&root_id=" + root_id + "&curr_path=" +  curr_path + "&" + hash.param;
	
//	var url = getAjaxUrl("cloud","doc_list1",query);
//	var data = {"label":"ptpfolderace","root_type":root_type} ;
//	//document.write(url+JSON.stringify(data));
//	$.ajax({
//	   type: "POST",
//	   dataType:"json",
//	   data:data,
//	   url: url,
//	   success: function(result){
//			if (result.status == undefined)
//			{
//				var permissions="";
//				if(root_type==1){
//					if (result.rows.length>0){
//						for (var i = 0; i < result.rows.length; i++) {
//							ace_data.push({"doc_type":DOC_FOLDER,"doc_id":result.rows[i].ptpfolderid,"power":result.rows[i].docace});
//						}
//					}
//				}else{
//					if (result.rows.length>0){
//						for (var i = 0; i < result.rows.length; i++) {
//							permissions+=result.rows[i].permissions; 
//						}
//					}
//					aces=permissions.split(",") ;
//					aces=aces.notempty() ;
//				}
//				//alert(JSON.stringify(ace_data));
//				//列出内容
//				
//				//重新按钮
//				init_cmd_btn();
//			}
//			else
//			{
//				myAlert(result.msg);
//			}
//	   }
//	});
	//得到权限
	//power = get_power();
	
	

}

function doc_dropzone()
{
//	var uploadfileurl=getAjaxUrl("cloud","doc_upload",query + "&root_type=" + root_type);
//	$('#doc_list').uploadfile({
//		url : uploadfileurl,
//		width : 500,
//		height : 50,
//		canDrag : true,
//		canMultiple : true,
//		success: function (response) {
//			var data = eval("(" + response + ")") ;
//			file_upload_callback(data);
//		},
//		error: function (fileName) {
//			showTips('err', fileName + '上传失败！');
//		},
//		complete : function () {
//			//alert('所有文件上传完毕');
//		}
//	});
				
    $("#doc_list").dropzone({
        url: getAjaxUrl("cloud","doc_upload",query + "&root_type=" + root_type),
		clickable: false,
        drop: function drop(e) {
          $("#upload_container").show();
        },
        addedfile: function (file) {
			//console.log('addedfile:'+file.upload.uuid+file.name+file.size);
			var _this2 = this;
			var itemData = {
				'fileID' : file.upload.uuid,
				'fileName' : file.name,
				'fileSize' : get_filesize(file.size)
			}

			var itemHTML = '<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel"><a href="javascript:void(0)">X</a></div>\
					<span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>\
					<div class="uploadify-progress">\
						<div class="uploadify-progress-bar"><!--Progress Bar--></div>\
					</div>\
				</div>';
			for ( var d in itemData) {
				itemHTML = itemHTML.replace(new RegExp(
							'\\$\\{' + d + '\\}', 'g'), itemData[d]);
			}
            //document.getElementById("upload_queue").appendChild(file.previewElement);
			$('#upload_queue').append(itemHTML);
            var removeFileEvent = function removeFileEvent(e) {
              //e.preventDefault();
              //e.stopPropagation();
			  //console.log('removeFile:'+file.status+'|'+Dropzone.UPLOADING+'|'+_this2.options.dictRemoveFileConfirmation);
              _this2.removeFile(file);
            };
			//document.getElementById(".cancel").addEventListener("click", removeFileEvent);
			$(".cancel").click(function(){
				removeFileEvent();
			})
        },
        removedfile: function removedfile(file) {
			//console.log('removedfile:'+file.status);
			$('#'+file.upload.uuid).remove();
//          if (file.previewElement != null && file.previewElement.parentNode != null) {
//            file.previewElement.parentNode.removeChild(file.previewElement);
//          }
//          return this._updateMaxFilesReachedClass();
        },
        sending: function(file, xhr, formData) {
            if(file.fullPath) formData.append("fullPath", file.fullPath);
			if(file.existname) formData.append("existname", file.existname);
			//console.log(getAjaxUrl("cloud","doc_upload",query + "&root_type=" + root_type)+"&fullPath="+file.fullPath); 
        },
        uploadprogress: function uploadprogress(file, progress, bytesSent) {
			//console.log('uploadprogress:'+file.upload.uuid+bytesSent);
			$('#' + file.upload.uuid).find('.data').html(
						' - ' + parseInt(progress) + '%');
			$('#' + file.upload.uuid).find('.uploadify-progress-bar').css('width',
					progress + '%');
        },
		success: function (file, response) {
			//$("#upload_container").hide();
			var data = eval("(" + response + ")") ;
			file_upload_callback2(data);
        },
        complete: function complete(file) {
			setTimeout(function() {
				if ($('#' + file.upload.uuid)) {
					$('#' + file.upload.uuid).fadeOut(500, function() {
						$(this).remove();
					});
				}
			}, 3 * 1000);
        },
		queuecomplete: function queuecomplete() {
			$("#upload_container").hide();
			},
		init: function () {
		   var _this=this;
			this.on("processing", function (file) {
				_this.options.url = getAjaxUrl("cloud","doc_upload",query + "&root_type=" + root_type);
			});
		},
        accept: function accept(file, done) {
		  var data = {'loginname':curr_loginname,'password':curr_password,'parent_type':parent_type,'parent_id':parent_id,'root_type':root_type,'root_id':root_id,'filename':file.name,'flag':'dropzone'} ;
		  //console.log('doc_checkExisting:'+JSON.stringify(data));
		  $.ajax({
			  type : 'POST',
			  dataType:"json",
			  async : false,
			  url : '/public/cloud.php?op=doc_checkExisting',
			  data : data,
			  success : function(result) {
				  //console.log('doc_checkExisting1:'+JSON.stringify(result));
				  switch(parseInt(result.status))
				  {
					  case 2:
						  if(result.msg){
							  var overwrite = confirm(langs.doc_exists_confirm.replace("{name}",file.name));
							  if (overwrite) {
								  file.existname = result.msg;
							  }
						  }
						  break;
				  }
			  }
		  });
          return done();
        },
        error: function (file, response) {
			err_upload_count++;
			$("#ok_upload_count").html(ok_upload_count);
			$("#err_upload_count").html(err_upload_count);
			$("#upload_container1").show();
			showTips('err', file.name + '上传失败！');
        }
    });
}

function path_valid(arr_url,i)
{
	if (i == undefined)
		i = arr_url.length-1 ; //最后
	var arr_item = arr_url[i].split("_") ;
	return (arr_item.length>1) ;
}

//得到解析名称
function path_getname(name)
{
	switch(name)
	{
		case "all":
			return langs.path_all;
		case "search":
			return langs.path_search;
		case "recent":
			return langs.path_recent;
		case "public":
			return langs.path_public;
		case "person":
			return langs.path_person;
		case "favitor":
			return langs.path_favitor;
		case "share":
			return langs.path_share;
		case "1":
			return langs.path_documents;
		case "2":
			return langs.path_picture;
		case "3":
			return langs.path_music;
		case "4":
			return langs.path_video;
		default:
			var arr = name.split("_");
			if (arr.length == 4)
				return decode64(arr[0]) ;
			else
				return decode64(name) ;
	}
}

function nav_getname(name)
{
	$(".nav-list").removeClass("nav-current");
	switch(name)
	{
		case "public":
			$(".nav-list").eq(0).addClass("nav-current");
			break;
		case "public/1":
			$(".nav-list").eq(1).addClass("nav-current");
			break;
		case "public/2":
			$(".nav-list").eq(2).addClass("nav-current");
			break;
		case "public/3":
			$(".nav-list").eq(3).addClass("nav-current");
			break;
		case "public/4":
			$(".nav-list").eq(4).addClass("nav-current");
			break;
		case "person":
			$(".nav-list").eq(5).addClass("nav-current");
			break;
		case "person/1":
			$(".nav-list").eq(6).addClass("nav-current");
			break;
		case "person/2":
			$(".nav-list").eq(7).addClass("nav-current");
			break;
		case "person/3":
			$(".nav-list").eq(8).addClass("nav-current");
			break;
		case "person/4":
			$(".nav-list").eq(9).addClass("nav-current");
			break;
		case "recent":
			$(".nav-list").eq(10).addClass("nav-current");
			break;
		default:
			if(name.indexOf("public")>-1) $(".nav-list").eq(0).addClass("nav-current");
			else if(name.indexOf("person")>-1) $(".nav-list").eq(5).addClass("nav-current");
			break;
	}
}

