    ...
    var content = tools.$('.weiyun-content')[0];
	var mainBottomRight = tools.$('.file-show')[0];
	var nav = tools.$('.lay-aside')[0];
	var main_header = tools.$('.nav')[0];
...
//	if(navBtn.onOff){                 //如果全选按钮添加上class,将所有元素都选中
//		//tools.addClass(navBtn,"g-btn-gray1");
//		treeMenu.style.display = 'block';
//	}else{                      //否则所有的文件为不选的状态
//		//tools.removeClass(navBtn,"g-btn-gray1");
...
///////////////////////////////////////////////////////////自适应高度函数///////////////////////////////////////////////////////////
    function changeSize(){
        ...
        mainBottomRight.style.width = clinetW - nav.offsetWidth - 10 + "px";  
		//alert(mainBottomRight.style.width);
		main_header.style.width = clinetW - nav.offsetWidth- 20 + "px";  
        content.style.height = clinetH - header.offsetHeight + "px"; 
        //treeMenu.style.height =  clinetH - main_header.offsetHeight - header.offsetHeight + "px";
		fileList.style.height =  clinetH - main_header.offsetHeight - header.offsetHeight - 40 + "px";
    }
...
//    }
    ...
    var docItem = tools.$('.doc_item', fileList); 
    // 给每个文件初始化事件绑定
//    tools.each(fileItem, function (item, index) {
//         fileHandle(item);
//    });
    ...
    function getCheckedItem() {
        ...
        tools.each(allCheckbox, function (checkbox, index) {
            if (tools.hasClass(checkbox, 'checked')) {
                arr.push(docItem[index]);
            }
        });
        return arr;
    }
	
	function get_ids2(id)
	{
		if ((id == undefined) || (id == "") || (id == "0"))
		{
			...
			if (id == "")
			{
				showTips('err',langs.get_ids_warning);
				return "" ;
			}
		}
		return id ;
    ...
    tools.addEvent(fileList, 'mousedown', function (ev) {
        var target = tools.getTarget(ev);
//		if ( tools.parents(target, '.upload-btn') ) return;
//		if ( tools.parents(target, '.beautiful-input') ) return;
        if ( tools.parents(target, '.file-edtor') ) return;
        disX = ev.clientX;
        disY = ev.clientY;

        // 鼠标移动
        tools.addEvent(fileList, 'mousemove', mouseMove)
    ...
    // 鼠标抬起
    function mouseUp() {
        tools.removeEvent(fileList, 'mousemove', mouseMove);
        tools.removeEvent(fileList, 'mouseup', mouseUp);
        if (newDiv) {
...
//tools.addEvent(navBtn,'mousedown',function(ev){
...
//})
/////////////////////////////////////////////////////////////显示导航 ///////////////////////////////////////////////////////////
//tools.addEvent(navBtn,'mouseup',function(ev){
...
//tools.addEvent(arrayBtn,'mousedown',function(ev){
//    	if (ev.stopPropagation) { 
//		// this code is for Mozilla and Opera 
//		ev.stopPropagation(); 
//		} 
...
//})
/////////////////////////////////////////////////////////////显示导航 ///////////////////////////////////////////////////////////
//tools.addEvent(arrayBtn,'mouseup',function(ev){
//	var target = ev.srcElement;
//	if (ev.stopPropagation) { 
//	// this code is for Mozilla and Opera 
//	ev.stopPropagation(); 
...
//	else
//		show_list();
////    if(arrayBtn.onOff){                 //如果全选按钮添加上class,将所有元素都选中
...
//	if(root_type==1&&parent_id=="0"&&filetype==0) return;
//    var target = ev.srcElement;
...
//	
//    checkboxs = tools.$(".checkbox",disContent);
//	selectRight = whoSelect();
////alert(selectRight.length +"|"+ checkboxs.length);
//    ev = window.event || arguments.callee.caller.arguments[0]; 
//    if(selectRight.length <= 1){
//        if( target ){
//            //显示自定义菜单
//			if(target.getAttribute("data-myid")=="Public") var roottype=1;
//			else var roottype=3;
//			renameBtn.style.display = 'block';
//			editBtn.style.display = 'block';
//			set_btn_enabled($("#downloadBtn",menu),can(DOCACE_DOWNLOAD,roottype,target.getAttribute("data-rootid")));	
//			set_btn_enabled($("#editBtn",menu),is_oos(target.getAttribute("data-name"))&&can(DOCACE_UPDATE,roottype,target.getAttribute("data-rootid")));
//			set_btn_enabled($("#delectBtn",menu),can(DOCACE_DELETE,roottype,target.getAttribute("data-rootid")));
//			set_btn_enabled($("#moveBtn",menu),can(DOCACE_MANAGE,roottype,target.getAttribute("data-rootid")));
//			set_btn_enabled($("#renameBtn",menu),can(DOCACE_RENAME,roottype,target.getAttribute("data-rootid")));
//			set_btn_enabled($("#shareBtn",menu),can(DOCACE_SEND,roottype,target.getAttribute("data-rootid")));
//			set_btn_enabled($("#permissionBtn",menu),(target.getAttribute("data-type")==DOC_FOLDER)&&can(DOCACE_VERSION,roottype,target.getAttribute("data-rootid")));
//            menu.style.display = 'block';
//            //给自定义菜单定位
//            menu.style.left = ev.clientX + 'px';
//			if(ev.clientY+menu.offsetHeight>disContent.offsetHeight) menu.style.top=(ev.clientY-menu.offsetHeight) + 'px';
//            else menu.style.top = ev.clientY + 'px';
//			
//	        allList = tools.$(".list",disContent); 
//            for(var i = 0; i < allList.length; i++){
//                tools.removeClass(allList[i],'active');
//				getFirstElement(allList[i]).style.visibility = 'hidden';   
//                tools.removeClass(getFirstElement(allList[i]),'conCheckbox');
//            }
//
//            tools.addClass(target,"active");
//			getFirstElement(target).style.visibility = 'visible'; 
//            tools.addClass(getFirstElement(target),"conCheckbox");
//        }
//    }else if(selectRight.length === checkboxs.length){
//		if(listtype === "recent" || listtype === "search") return ;
//		set_btn_enabled($("#downloadBtn",menu),can(DOCACE_DOWNLOAD,root_type));	
//		set_btn_enabled($("#delectBtn",menu),can(DOCACE_DELETE,root_type));
//		set_btn_enabled($("#moveBtn",menu),can(DOCACE_MANAGE,root_type));
//		set_btn_enabled($("#shareBtn",menu),can(DOCACE_SEND,root_type));
//		set_btn_enabled($("#permissionBtn",menu),false);
//		//set_btn_enabled($("#renameBtn",menu),!(root_type==1&&(!can(DOCACE_CREATE,root_type))));
...
//    }else{      
//
//        var targetBox = getFirstElement(target);
//        
//        if( tools.hasClass(getFirstElement(target),"conCheckbox") ){
//            //////当前加上class
//		    if(listtype === "recent" || listtype === "search") return ;
//            tools.addClass(target,"active");
//			getFirstElement(target).style.visibility = 'visible'; 
