...
function init_cmd_btn()
{
	...
	if (label == "recent"||label == "search"||file_type!=0||(root_type==1&&parent_id=="00000000"))
		return ;
...
function init_cmd_btn_container(container)
{
	...
	if (label == "recent"||label == "search"){
	  $(".doc_item").each(function(){
		  ...
		  set_btn_enabled($(".btn_doc_delete",container),!(root_type==1&&(!can(DOCACE_CREATE,root_type))));
		  set_btn_enabled($(".btn_folder_create",container),!(root_type==1&&(!can(DOCACE_CREATE,root_type))));
		  set_btn_enabled($(".btn_doc_move",container),!(root_type==1&&(!can(DOCACE_CREATE,root_type))));
		  set_btn_enabled($(".btn_doc_edit",container),!(root_type==1&&(!can(DOCACE_CREATE,root_type))));
	}

	if (label == "recent"||label == "search"||file_type!=0) $(".btn_cmd",container).hide();
...
{
	if (result.status == undefined){
		if(file_type==2) doc_set_html(result.rows,2);
	}
	else
		myAlert(result.msg);
}
...
{
	var url = "http://" + site_address + "/cloud/include/doceditor.php?OnlineID="+item_id ;
	window.clipboardData.setData("Text",url);
	myAlert("复制链接成功"); 
}
...
function doc_move(item_id,e)
{
	...
	dialog("move","移动到","doc_move.html?root_id=" + root_id) ;
}
...
function doc_delete(item_id)
{
	...
	if (window.confirm("你确定要删除" + name + "吗？") == false)
		return ;
...
{
	var doc_item = $("#" + id);
	var url = "root_id=" + $(doc_item).attr("data-rootid") + "&parent_type=" + parent_type + "&parent_id=" + parent_id +
...
{
	var url = "http://" + site_address + get_download_url(id) + "&loginname=" + curr_loginname + "&password=" + curr_password ;
	...
	var html =  "<div><img src='/public/qrcode.html?size=3&encode=1&text=" + escape(url) + "'></div>" +
				...
				"<div style='padding:5px;'><input type='button' class='btn btn_primary' value='关闭' data-dismiss='modal' aria-hidden='true' /></div>" ;
...
function doc_rename1(id,e)
{
	...
	var value = $(doc_item).attr("data-name") ;
	$(doc_item).attr("data-oldvalue",value);
	$(doc_item).attr("data-oldvalue1",'<a href="http://'+site_address+'/cloud/include/doceditor.php?OnlineID='+$(doc_item).attr("data-id")+'&loginname='+curr_loginname+'&password='+curr_password+'" target="_blank">'+value+'</a>');
...
function doc_rename_save1(doc_item,ipt)
{
	...
	$.ajax({
	   ...
	   success: function(result){
			if (result.status)
				$(ipt).parent().html('<a href="http://'+site_address+'/cloud/include/doceditor.php?OnlineID='+$(doc_item).attr("data-id")+'&loginname='+curr_loginname+'&password='+curr_password+'" target="_blank">'+$(ipt).val()+'</a>') ;
			else
			{
				$(ipt).parent().html($(doc_item).attr("data-oldvalue1")) ;
				myAlert(result.msg);
...
{
	dialog("folder","新增目录","folder_edit.html") ;
}

function doc_create(doctype)
{
	if(doctype==1) var Stext="template\\new.docx";
	else var Stext="template\\new.xlsx";
	var url = getAjaxUrl("cloud","doc_onlinefile_save",query);
	var data = {"usname":escape(curr_myfcname),"filename":escape(getFullFileName(Stext)),"filesize":0,"target_file":escape(Stext)} ;
	//document.write(url+JSON.stringify(data));
	$.ajax({
	   type: "POST",
	   dataType:"json",
...
function doc_restore()
{
	...
	if (window.confirm("确定将文档恢复到第" + $(doc_item).attr("data-sid") + "版吗？") == false)
			return ;
...
function get_select_name(ids)
{
	...
	if (arr_id.length>1)
		name = name + "...(" + arr_id.length + ")个文件(文件夹)" ;
	return name ;
}
...
//判断是否在aces中
function get_power(op,roottype)
{
	var s;
	//因为路径是从跟开始，所以这个倒序
	if (roottype == 1){
...
function loadTree(container_id)
{
    ...
    var url = getAjaxUrl("cloud","get_root_tree","root_id=00000000") ;
	//document.write(url);
