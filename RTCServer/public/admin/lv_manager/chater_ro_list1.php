...
<!--							<td><a href="javascript:void(0);" class="icon-check${status}" data-value="${status}" action-data="${typeid},'status',this"  action-type="dataList.setSwitch"></a></td>-->
							...
							<?php }?>
								<a href="javascript:void(0);" onclick="chater_ro_edit1('${typeid}','${typename}')"><?=get_lang("btn_change")?></a>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        chater_ro_list_init1();
    })
	
	function chater_ro_list_init1()
	{
...
//            where = getWhereSql(where,"(subject like '%" + key + "%')") ;
		where = getWhereSql(where,"<?=$where ?>") ;
		//alert(where);
		return where ;
	}
	
	function chater_ro_edit1(id,_name)
	{
