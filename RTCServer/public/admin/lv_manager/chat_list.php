...
<body class="body-frame">
    <?php require_once("../include/header.php");?>
				<div class="page-header"><h1><?=get_lang('lvchat_list_title')?></h1></div>
				...
				<div class="searchbar">
                    ...
                    <div class="pull-right">
				      ...
				      <select  id="field" class="form-control pull-left ws mr" style="width:auto;">
				        ...
				        <option value="username"><?=get_lang('lvchat_list_field_name')?></option>
				        <option value="userid"><?=get_lang('lvchat_list_field_userid')?></option>
				        <option value="chatid"><?=get_lang('lvchat_list_field_chatid')?></option>
				         <option value="ip"><?=get_lang('lvchat_list_ip')?></option> 
				      </select>
                      <input type="text" class="form-control pull-left ws mr"  id="key" placeholder="<?=get_lang('lvchat_list_txt_keywords')?>" />
                      <input type="text" class="form-control datepicker pull-left" data-mask="date"  id="dt1" value="" data-date-format="yyyy-mm-dd"  style="width:120px;">
                      <span class="pull-left" style="line-height:34px;padding:0px 5px;"><?=get_lang('txt_to')?></span>
                      <input type="text" class="form-control datepicker pull-left mr"  data-mask="date" id="dt2" value="" data-date-format="yyyy-mm-dd" style="width:120px;">
                ...
                <table id="datalist" class="table table-hover data-list" data-obj="livechat_kf" data-tmpid="tmpl_list" data-table="lv_chat" data-fldid="chatid" data-fldname="chatid"  data-fldlist="*" data-fldsort="chatid desc" data-fldsortdesc="chatid asc">
                    <thead>
                        <tr>
                        	<td style="width:20px"><input type="checkbox"  name="chk_All" onClick="checkAll('chk_Id',this.checked)"></td>
                            <td data-sortfield="chatid"><?=get_lang('lvchat_list_chatid')?></td>
                            <td data-sortfield="groupid"><?=get_lang('lvchat_list_groupname')?></td>
                            <td data-sortfield="chater"><?=get_lang('lvchat_list_account')?></td>
                            <td data-sortfield="userid"><?=get_lang('lvchat_list_userid')?></td>
                            <td data-sortfield="username"><?=get_lang('lvchat_list_username')?></td>
                            <td data-sortfield="ip"><?=get_lang('lvchat_list_ip')?></td>
                            <td><?=get_lang('lvchat_list_first_inouttimes')?></td>
                            <td><?=get_lang('lvchat_list_inouttimes')?></td>
                            <td><?=get_lang('lvchat_list_chat_times')?></td>
                            <td data-sortfield="rate"><?=get_lang('lvchat_list_rate')?></td>
                            ...
                            <td data-sortfield="intime"><?=get_lang('lvchat_list_intime')?></td>
                            <td data-sortfield="outtime"><?=get_lang('lvchat_list_outtime')?></td>
                            <td data-sortfield="status" style="width:60px;"><?=get_lang('lvchat_list_attach')?></td>
                            <td></td>
                        </tr>
                    </thead>
                ...
                <script type="text/x-jquery-tmpl" id="tmpl_list">
                    <tr id="${chatid}" class="status-${status}">
                        ...
                        <td style="width:100px">
							<a href="/admin/report/livechat_list2.html?chatid=${chatid}"><?=get_lang('lvchat_list_record')?></a>
							<a href="/admin/report/livechat_list.html?user1=${chater}&user2=${userid}"><?=get_lang('lvchat_list_allrecord')?></a>
                            <a href="file_list.html?field=chatid&key=${chatid}"><?=get_lang('lvchat_list_attach')?></a>
							<a href="javascript:void(0);" action-data="'${myid}','${userid}','${ip}'" action-type="sure_blacklist" ><?=get_lang('lvchat_list_blackip')?></a>
                        </td>
                    </tr>
                </script>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		if (isenable != "") $("#isenable").val(isenable);
		if (dt1 != "") $("#dt1").val(dt1);
		if (dt2 != "") $("#dt2").val(dt2);
    ...
    function formatData(data)
    {
        for (var i = 0; i < data.length; i++) {
            data[i].status_name = get_status_name(data[i].status) ;
			...
			data[i].outtime = toDate(data[i].outtime,1) ;
        }
        return data ;
    }
    ...
    function get_status_name(status)
    {
        ...
        {
            case 0:
                return "<?=get_lang('lvchat_list_return_case0')?>" ;
            case 1:
                return "<?=get_lang('lvchat_list_return_case1')?>" ;
            case 2:
                return "<?=get_lang('lvchat_list_return_case2')?>" ;
            default:
                return "" ;
        }
    }
	...
	function sure_blacklist(myid,userid,ip){
		var url = getAjaxUrl("user","set_silence");
		var param = {"MyID":myid,"YouID":userid,"TO_IP":ip,"To_Type":3,"Silence":1} ;
		...
		$.ajax({
		   ...
		   success: function(data){
				myAlert(langs.livechat_blackip_create_ok);
		   }
		});
	}
</script>
