function user_ro_member(_objId)
{
...
function user_ro_edit(id,_name)
{
	...
	var title = (id == ""?langs.livechat_user_ro_create:langs.livechat_user_ro_edit) ;
	var url = "../lv_manager/user_ro_edit.html?" + (id == ""?"":"groupid=" + id ) + "&groupname=" + escape(_name) ;
	dialog("user_ro",title ,url,{width:0,height:350,isClear:true}) ;
}

var id = "" ;
function user_ro_delete(_id)
{
...
function saveForm()
{
    if ($("#typename").val() == "")
    {
        setElementError($("#typename"),langs.livechat_user_ro_name_require);
        return false ;
    }
    ...
    if(itemIndex <= 0 )
	{
    	myAlert(langs.livechat_user_ro_order_require_integer);
    	return;
	}
    dataForm.save();
}
