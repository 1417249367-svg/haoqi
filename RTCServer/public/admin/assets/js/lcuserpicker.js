...
function addMemberByInput()
{
	...
	addMember(arrItem[0],arrItem[1],arrItem[2]);
}
...
function addMember(userId,userName,loginName)
{
	...
	var row = $("<tr id='user_" + userId + "' data='" + userId + "'><td class='col_name'>" + userName + "</td><td class='col_loginname'>" + loginName + "</td><td><a href='#' onclick='deleteMember(" + userId + ")'>"+langs.btn_delete+"</a></td></tr>") ;
...
function deleteMember(userId)
{
	...
	Smsn=my_ids.split(",");
	if(Smsn.length<=1){
		myAlert(langs.livechat_warning);
		return ;
	}
	$("#user_" + userId).remove();
}
