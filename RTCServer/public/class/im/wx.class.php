...
//			 exit; 
    ...
    public function sendminiprogramSubscribeMsg($myid,$youid,$usertext)
    {
		...
		$postData = array(
            ...
            'data'    => array (
				...
				'thing3'    => array (
					'value' => $reader -> decodeUserText($usertext),
					'color' => "#173177"
				)
            )
        );
    ...
    public function sendSubscribeClotmsg($myid,$youid,$usertext,$typename,$pid)
    {
		$accessToken = $this->getAccessToken();
		$reader = new MsgReader();
		if(WECHAT_SUBSCRIBE){
			...
			$postData=array( 
				'touser'=>strval($youid), //要发送给用户的openId
				'template_id'=>WECHAT_SUBSCRIBE_ID2,//改成自己的模板id，在微信接口权限里一次性订阅消息的查看模板id
				'page'=>RTC_SERVER_AGENT.'/kefu.html?typeid=' . $pid . '&userid='.$youid,
				'data'    => array (
					'thing1'    => array (
						'value' => $myid
					),
			 ...
			 $url = "https://api.weixin.qq.com/cgi-bin/message/template/subscribe?access_token=".$accessToken;
			  $postData=array( 
				  'touser'=>strval($youid), //要发送给用户的openId
				  'template_id'=>WECHAT_SUBSCRIBE_ID,//改成自己的模板id，在微信接口权限里一次性订阅消息的查看模板id
				  'url'=>RTC_SERVER_AGENT.'/kefu.html?typeid=' . $pid . '&userid='.$youid,
				  'scene'=>"888",
				  'title'=>"您有一条新的群聊信息",  //标题
				  'data'=>array(
					  'content'=>array(
						  'value' => "客服有新的群聊信息,快去看看吧!",
						  'color'=>"#173177"
					  )
				  )
			  );
    ...
    public function sendCustomerClotMsg($myid,$youid,$usertext,$typename,$pid)
    {
		$accessToken = $this->getAccessToken();
        $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=".$accessToken;
        // 不指定某个客服回复
...
//            'touser'  => $youid,
//            'template_id' => WECHAT_MOBAN_ID,
//			'url'=>RTC_SERVER_AGENT.'/kefu.html?typeid=' . $pid . '&userid='.$youid. '&moban=1',
//            'data'    => array (
//				'first'    => array (
//					'value' => "您有一条新的群聊信息!",
//					'color' => "#173177"
...
//			 exit; 
    ...
    public function sendminiprogramSubscribeClotMsg($myid,$youid,$usertext,$typename,$pid)
    {
		$accessToken = $this->getAccessToken1();
        $url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$accessToken;
		//$url = "https://api.weixin.qq.com/cgi-bin/message/wxopen/template/uniform_send?access_token=".$accessToken;
    ...
    public function getAccessToken1()
    {
		...
		if (isset($_SESSION['access_token']) && time() < $_SESSION['expires_time']) {
            ...
            $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".WECHAT_APPID1."&secret=".WECHAT_SERCET1;
