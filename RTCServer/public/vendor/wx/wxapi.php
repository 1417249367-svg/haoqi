...
{
    public $appid  = WECHAT_APPID;
    public $sercet = WECHAT_SERCET;
    public $token  = WECHAT_TOKEN;
...
//                            $this->sendCustomerMsg($postObj);
							...
							$db -> addParamField("subscribe",1);
							$db -> addParamWhere("userid", $postObj->FromUserName);
							return $db -> update();
                        }elseif ($postObj->Event == 'unsubscribe') {
							$db = new Model("lv_user");
							$db -> addParamField("subscribe",0);
							$db -> addParamWhere("userid", $postObj->FromUserName);
							return $db -> update();
						}
                        break;
