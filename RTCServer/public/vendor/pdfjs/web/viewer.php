<?php
	...
	$db = new DB();
	$ishidden = 0;
	$url = getRootPath()."/public/cloud.html?op=getpdffile3&formfiletype=".g("FormFileType")."&col_id=".g("col_id",0)."&name=".g("filename")."&loginname=".phpescape(CurUser::getLoginName())."&password=".CurUser::getPassword();
	if(g("file")!="") $url = g("file");
	switch ((int)g("formfiletype")) {
		case 1 :
			$sql = " select * from PtpFile where Msg_ID='" . g("col_id") . "'";
			$row = $db -> executeDataRow($sql);
			if ((int)$row["to_type"] == 1){
				if (! DocAce::can1(DOCACE_DOWNLOAD,DOC_FOLDER,$row["ptpfolderid"],$root_id,1)) $ishidden = 1;
			}else{
				if (! DocAce::can3(DOCACE_DOWNLOAD,$parent_type,$parent_id,$root_id)) $ishidden = $ishidden = 1;
			}
			break;
		case 2 :
			if (! DocAce::can3(DOCACE_DOWNLOAD_LEAVEFILE,$parent_type,$parent_id,$root_id)) $ishidden = 1;
			break;
		case 4 :
			if (! DocAce::can3(DOCACE_DOWNLOAD_CLOTFILE,$parent_type,$parent_id,$root_id)) $ishidden = 1;
			break;	
	}
?>
<!DOCTYPE html>
...
<script src="../build/pdf.js"></script>
  ...
  <script src="viewer.js?ver=20231124"></script>
